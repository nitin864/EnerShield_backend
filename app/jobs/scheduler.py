...
def run_all_ingestion_jobs():
    """
    Wraps both ingestion jobs with individual error handling — if NewsAPI
    ingestion throws, EIA ingestion should still run. One flaky source
    should never take down the whole pipeline (same "never let the demo
    break on one dead call" principle from the build guide).
    """
    try:
